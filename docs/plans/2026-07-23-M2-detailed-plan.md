...
| 2.3.6 | Per-row content rejection: a well-formed response that fails **per-row** (not whole-batch) validation increments that row's in-memory attempt count (F-D); cap 3 → `messages.embed_status='failed'` (terminal, drops out of the set-difference query). The sidecar protocol is batch-only (ADR 0024, no per-item rejection signal), so a whole-batch failure is first treated as a transport failure (2.3.5); **isolating the single bad row is done by the bisection loop (2.3.11)** — which shrinks the batch until the offender is alone and this per-row cap-3 can fire (this replaces the earlier "document the limitation" hand-wave — review v3-B1). **v1 accepted behavior (M2.3 code-review C1, reconciled M2.6.7):** with the batch-only protocol, a solo-batch `embed()` failure is **ambiguous** (content rejection vs. transport failure), so per ADR 0015's safe default it's treated as a **transport failure** — the row stays `pending`, never `failed`, no attempt increment. Consequence: the cap-3 → `failed` mechanism is **dormant scaffolding** in v1 (a genuine poison-pill row stays `pending` indefinitely, FTS5 lexical unaffected). Activating it requires adding a positive per-item rejection signal to ADR 0024 (e.g. `rejected_indices` array) — deferred. | Worker test: fake `Embedder` rejects one specific text 3 times in solo-batches → row flips to `failed` on the 3rd attempt and disappears from subsequent set-difference results (end-to-end poison-pill isolation is 2.3.11's test). **NOTE (M2.6.7):** the original 2.3.6 test intent is deferred; the current implementation treats solo-batch failures as transport (row stays `pending`). |
...
| 2.3.11 | **Poison-pill batch bisection (review v3-B1):** `embed` is batch-only (ADR 0024, no per-item error), so one un-embeddable row in a batch of 64 errors the whole call → 2.3.5 treats it as transport-failure → the deterministic oldest-first `fetch_pending_embeddings` (`ORDER BY m.id`) re-selects the SAME batch every cycle = **livelock** (the offender never reaches `failed`; no later row ever drains; 2.3.8 eventually trips account-wide). Fix: after **K consecutive whole-batch failures of the same message-id set**, halve the batch and retry, converging to solo-batches — the innocent rows drain meanwhile. **v1 accepted behavior (M2.3 code-review C1, reconciled M2.6.7):** because solo-batch failures are treated as transport (2.3.6 note), bisection isolates the offender to `pending` (never wrongly `failed` during a sidecar outage), but a genuinely-unembeddable row also stays `pending` indefinitely rather than retiring to `failed`. This is the dormant-scaffolding consequence — lexical search unaffected. Pure whatsrust-side retry; no ADR 0024 change. (Formalized in ADR 0038.) | Worker test: a batch of 64 with ONE poisoned text → the other 63 drain within N cycles AND the poisoned row reaches `failed` within 3 solo attempts (proves no livelock). **NOTE (M2.6.7):** the original 2.3.11 test intent (offender reaches `failed`) is deferred; v1 treats solo-batch failures as transport, so offenders stay `pending`. |
...
| 2.4.5 | Multilingual default (ADR 0018): document the recommended `WHATSRUST_EMBEDDER_CMD`/model choice as a multilingual model in `.env.example`, with a one-line rationale (CJK/Thai semantic coverage where `unicode61` FTS5 degrades). whatsrust itself stays language-neutral — no code branches on language. **Model rollout decided (2026-08-30, ADR 0039):** MVP `paraphrase-multilingual-MiniLM-L12-v2` (384-dim) via a Python sidecar → evolve to `bge-m3` (1024-dim); **prefix-free models only** (e5 excluded — its query/passage prefixes need a protocol change, ADR 0024/0039). The real sidecar is a separate deliverable — build spec `docs/plans/2026-08-30-embedder-sidecar-mvp.md`. | `.env.example` recommends the MVP-first prefix-free path + rationale (done). |
